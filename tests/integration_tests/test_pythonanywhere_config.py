...
def test_settings(tmp_project):
    """Verify there's a PythonAnywhere-specific settings section.
    ...
    again.
    """
    hf.check_reference_file(tmp_project, "blog/settings.py", "dsd-pythonanywhere")
...
def test_gitignore(tmp_project):
    ...
    hf.check_reference_file(tmp_project, ".gitignore", "dsd-pythonanywhere")


# --- Test PythonAnywhere-specific files ---
...
def test_log_dir(tmp_project):
    ...
    assert (
        "INFO: Deployment target: PythonAnywhere" in log_file_text
        or "INFO: Deployment target: PythonAnywhere" in log_file_text
    )
    assert "INFO:   Using plugin: dsd_pythonanywhere" in log_file_text
